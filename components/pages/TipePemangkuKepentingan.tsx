...
import { getTipePemangkuKepentingan, addTipePemangkuKepentingan, updateTipePemangkuKepentingan, deleteTipePemangkuKepentingan } from '../../services/database';
import type { TipePemangkuKepentingan } from '../../types';

type TipeData = Omit<TipePemangkuKepentingan, 'id'>;

const TipePemangkuKepentinganPage: React.FC = () => {
    const [data, setData] = useState<TipePemangkuKepentingan[]>([]);
    const [loading, setLoading] = useState(true);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<TipePemangkuKepentingan | null>(null);
    const [itemToConfirmDelete, setItemToConfirmDelete] = useState<TipePemangkuKepentingan | null>(null);
    const [toastMessage, setToastMessage] = useState<string | null>(null);

    const loadData = useCallback(async () => {
        setLoading(true);
        try {
            const result = await getTipePemangkuKepentingan();
            setData(result);
        } catch (error) {
    ...
    const handleOpenModal = (item: TipePemangkuKepentingan | null) => {
        setEditingItem(item);
        setIsModalOpen(true);
    };
    ...
    const handleSave = async (formData: TipeData & { id?: number }) => {
        setIsSubmitting(true);
        try {
            if (formData.id) {
                await updateTipePemangkuKepentingan(formData.id, formData);
                showToast('Tipe berhasil diperbarui.');
            } else {
                await addTipePemangkuKepentingan(formData);
                showToast('Tipe berhasil ditambahkan.');
            }
            await loadData();
        } catch (error) {
            console.error("Failed to save data:", error);
            showToast('Gagal menyimpan tipe.');
        } finally {
    ...
    const handleDeleteRequest = (item: TipePemangkuKepentingan) => {
        handleCloseModal();
    ...
    const handleConfirmDelete = async (id: number) => {
        setIsSubmitting(true);
        try {
            await deleteTipePemangkuKepentingan(id);
            showToast('Tipe berhasil dihapus.');
            await loadData();
        } catch (error) {
            console.error("Failed to delete data:", error);
            showToast('Gagal menghapus tipe.');
        } finally {
            setIsSubmitting(false);
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-800">Tipe Pemangku Kepentingan</h1>
                <button
            ...
            <div className="bg-white p-4 rounded-lg shadow-md overflow-x-auto">
                <table className="w-full text-sm text-left text-gray-500">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            <th scope="col" className="px-6 py-3">Nama Tipe</th>
                        ...
                        {loading ? (
                            ...
                            data.map(item => (
                                <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-6 py-4 font-medium text-gray-900">{item.namaTipe}</td>
...
interface ModalProps {
    item: TipePemangkuKepentingan | null;
    onSave: (data: TipeData & { id?: number }) => void;
    onClose: () => void;
    onDeleteRequest: (item: TipePemangkuKepentingan) => void;
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg">
                <h2 className="text-2xl font-bold text-gray-800 mb-6">{item ? 'Ubah Tipe' : 'Tambah Tipe Baru'}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="namaTipe" className="block text-sm font-medium text-gray-700">Nama Tipe</label>
                        <input type="text" id="namaTipe" name="namaTipe" value={formData.namaTipe} onChange={handleChange} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                    </div>
                    <div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
            <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md animate-fade-in-up">
                ...
                <p className="mt-4 text-gray-600">
                    Apakah Anda yakin ingin menghapus tipe <strong className="text-gray-800">"{item.namaTipe}"</strong>? Tindakan ini tidak dapat diurungkan.
                </p>
...
export default TipePemangkuKepentinganPage;